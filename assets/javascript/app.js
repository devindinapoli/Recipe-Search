...
var queryUrl =
  "https://api.yummly.com/v1/api/recipes?" + apiKey + "&requirePictures=true";
  ...
  for (var i = 0; i < array.length; i++) {
    var recipeCard = $(
      "<div class='col s12 m6 l4 recipe-card'>" +
        ...
        "<img src=" +
        array[i].images[0].hostedMediumUrl +
        ">" +
        ...
        array[i].source.sourceRecipeUrl +
        "'>" +
        array[i].source.sourceDisplayName +
        "</a>" +
        "<br><br><i class='material-icons fav-icon' id=" +
        array[i].id +
        ">favorite</i>" +
        "</div></div>"
    );
...
var recipeCall = function() {
  ...
  for (var i = 0; i < recipeArray.length; i++) {
    var newUrl =
      "https://api.yummly.com/v1/api/recipe/" + recipeArray[i] + "?" + apiKey;
    $.ajax({
      url: newUrl,
      method: "GET"
...
$("#zip-button").on("click", function(event) {
  ...
  var newMap = $("#map").attr(
    "src",
    "https://www.google.com/maps/embed/v1/search?key=AIzaSyBG5a2EUHZpq-aoy20slw4V_TpzY2ZqIMc&q=grocery+stores+" +
      zip
  );
  console.log(newMap);
  //Devin's API
...
$("#dish-btn").on("click", function(event) {
  ...
  if ($("#include-ingredient").val() != "") {
    var ingredients = $("#include-ingredient")
      .val()
      .trim();
  ...
  if ($("#exclude-ingredient").val() != "") {
    var exclude = $("#exclude-ingredient")
      .val()
      .trim();
  ...
  if ($("select#course-dropdown option:selected").val() != "none") {
    search +=
      "&allowedCourse[]=" + $("select#course-dropdown option:selected").val();
  }
  ...
  if ($("select#cuisine-dropdown option:selected").val() != "none") {
    search +=
      "&allowedCuisine[]=" + $("select#cuisine-dropdown option:selected").val();
  }

  // checking for diet input
  if ($("select#diet-dropdown option:selected").val() != "none") {
    search +=
      "&allowedDiet[]=" + $("select#diet-dropdown option:selected").val();
  }
  ...
  $("#map").attr(
    "src",
    "https://www.google.com/maps/embed/v1/search?key=AIzaSyBG5a2EUHZpq-aoy20slw4V_TpzY2ZqIMc&q=grocery+stores+near+me"
  );
...
$("#fav-btn").on("click", function(event) {
  ...
  for (var i = 0; i < favArray.length; i++) {
    var newUrl =
      "https://api.yummly.com/v1/api/recipe/" + favArray[i] + "?" + apiKey;
    $.ajax({
      url: newUrl,
      method: "GET"
    }).then(function(response) {
